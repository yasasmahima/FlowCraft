        ...
        with h5py.File(output_dir_ / f'{scene_id}.h5', 'r+') as f:
            for ts in timestamps:
                ...
                if key not in f.keys():
                    ...
                    continue
                group = f[key]
                mask = pd.read_feather(
                    Path(mask_dir) / f"{data_mode}-masks" / scene_id / f"{key}.feather").to_numpy().astype(bool)
...
def compute_sceneflow(data_dir: Path, log_id: str, timestamps: Tuple[int, int]) -> Dict[str, Union[np.ndarray, SE3]]:
    ...
    poses = [convert_pose_dataframe_to_SE3(filtered_log_poses_df.loc[filtered_log_poses_df["timestamp_ns"] == ts]) for
             ts in timestamps]
...
def process_log(data_dir: Path, log_id: str, output_dir: Path, n: Optional[int] = None):
    def create_group_data(group, pc, gm, pose, flow_0to1=None, flow_valid=None, flow_category=None,
                          flow_1_category=None, ego_motion=None):
        ...
        if flow_0to1 is not None:
            ...
            group.create_dataset('flow_category_indices_1', data=flow_1_category.astype(np.uint8))
    ...
    if (len(os.listdir(log_map_dirpath)) < 3):
        print(
            f'{log_map_dirpath} needed by 3 to find the ground layer, check if you are using the correct *sensor* dataset')
...
def main(
        argo_dir: str = "/home/yasas/UNSW/Adv_Temporal/SceneFlow/argoverse2/data",
        output_dir: str = "/home/yasas/UNSW/Adv_Temporal/SceneFlow/argoverse2/processed_test",
        av2_type: str = "sensor",
        data_mode: str = "val",
        mask_dir: str = "/home/yasas/UNSW/Adv_Temporal/SceneFlow/argoverse2/masks",
        nproc: int = (multiprocessing.cpu_count() - 10)
):
