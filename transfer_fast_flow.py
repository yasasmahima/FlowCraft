...
from scripts.utils.av2_eval import CATEGORY_TO_INDEX, CRITICAL_OBJECTS, compute_accuracy, compute_true_positives, \
    compute_true_negatives, compute_false_positives, compute_false_negatives
...
def main(cfg):
    ...
    run_attack(mymodel, DataLoader(HDF5Dataset(cfg.dataset_path + f"/{cfg.av2_mode}", eval=True), batch_size=1,
                                   shuffle=False))
...
def run_attack(model, data_loader):
    torch.set_grad_enabled(True)
    print(len(data_loader.dataset))
    f = open("pgd_attack_eval.txt", "a")
    results = open("transfer_fast_flow_results_cos_pgd.txt", "a")
    chamfer_list = []
    EPE_attack = []
    EPE_attack_dynamic = []
    EPE_attack_static = []
    ...
    print("==============Results DeFlow->FastFlow CosPGD===============")
...
def calculate_iou(dts, gts):
    tp = compute_true_positives(dts, gts)
    tn = compute_true_negatives(dts, gts)
    fp = compute_false_positives(dts, gts)
    fn = compute_false_negatives(dts, gts)
...
def penalize_loss(res_dict, original_epe, classes_critical):
    original_epe = original_epe[classes_critical]
    pred = res_dict['est_flow'].cuda()
    gt = res_dict['gt_flow'].cuda()
    gt = gt[classes_critical]
    pred = pred[classes_critical]
    error_attack = torch.linalg.vector_norm(pred - gt, dim=-1)
    loss = 1 + torch.nn.functional.relu(
        original_epe - error_attack) * 1  # This gives more weight to the instances where attacked end point error is lower than the original FastFlow = 1000 DeFlow = 1
    return loss
