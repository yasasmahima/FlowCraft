...
def compute_accuracy(
    ...
    relative_error_inlier = np.less(relative_error, distance_threshold).astype(bool)  #Check both absoulte and realative errors
    ...
    for cats_name in BUCKETED_METACATAGORIES:
        ...
        if cats_name == "BACKGROUND":
            ...
            storage_error_matrix.append(
                BaseSplitValue(cats_name, error_flow[mask].mean(), gt_speeds[mask].mean(), (0.0, 0.04), mask.sum()))
            continue
        for min_speed_threshold, max_speed_threshold in speed_thresholds:
            ...
            storage_error_matrix.append(
                BaseSplitValue(cats_name, avg_epe, avg_speed, (min_speed_threshold, max_speed_threshold), count_pts))
