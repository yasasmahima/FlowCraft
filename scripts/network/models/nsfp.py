...
class NSFP(nn.Module):
    def __init__(self, filter_size=128, act_fn='relu', layer_size=8, \
        ...
        self.my_chamfer_fn = ChamferDistance()
    ...
    def optimize(self, pc0, pc1):
        ...
        for _ in range(self.iteration_num):
            ...
            dist1, dist2 = self.my_chamfer_fn(pc0_to_pc1.unsqueeze(0), pc1.unsqueeze(0))
            forward_loss = (torch.mean(dist1)) + (torch.mean(dist2))
            self.timer[2][0].stop()

            self.timer[2][1].start("Inverse Loss")
            dist1, dist2 = self.my_chamfer_fn(est_pc1_to_pc0.unsqueeze(0), pc0.unsqueeze(0))
            inverse_loss = (torch.mean(dist1)) + (torch.mean(dist2))
            self.timer[2][1].stop()
            self.timer[2].stop()
    ...
    def forward(self, batch):
        ...
        for batch_id in range(batch_sizes):
            ...
            pc0 = batch["pc0"][batch_id].cuda()
            pc1 = batch["pc1"][batch_id].cuda()
