...
def collate_fn_pad(batch):
    ...
    if 'flow' in batch[0]:
        ...
        flow_category_indices_1 = torch.nn.utils.rnn.pad_sequence(
            [batch[i]['flow_category_indices_1'][~batch[i]['gm1']] for i in range(len(batch))], batch_first=True)
        res_dict['flow'] = flow
        res_dict['flow_is_valid'] = flow_is_valid
        res_dict['flow_category_indices'] = flow_category_indices
        res_dict['flow_category_indices_1'] = flow_category_indices_1
    ...
    def __getitem__(self, index_):
        ...
        with h5py.File(os.path.join(self.directory, f'{scene_id}.h5'), 'r') as f:
            ...
            if 'flow' in f[key]:
                ...
                flow_category_indices_1 = torch.tensor(f[key]['flow_category_indices_1'][:])
                res_dict['flow'] = flow
                res_dict['flow_is_valid'] = flow_is_valid
                res_dict['flow_category_indices'] = flow_category_indices
                res_dict['flow_category_indices_1'] = flow_category_indices_1
